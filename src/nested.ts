...
export function findQuestion(
    ...
    return foundQuestion || null; // Convert undefined to null if the question is not found
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (acc, question) => (question.published ? acc + question.points : acc),
        0
    );
}
...
export function addNewQuestion(
    ...
    return [...questions, newBlankQuestion];
}
...
export function duplicateQuestionInArray(
    ...
    return questions.flatMap((question) => {
        if (question.id === targetId) {
            ...
            return [question, duplicatedQuestion];
        } else {
            return [question];
        }
    });
